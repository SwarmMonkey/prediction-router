...
export default function AnalyzeButton({ market, onAnalysisComplete, onAnalyzeStart, hideResult = false }: AnalyzeButtonProps) {
  ...
  const handleAnalyze = async () => {
    ...
    if(!market || !market.markets || !Array.isArray(market.markets) || market.markets.length === 0){
    ...
    if(market.markets.length === 1){
      analysisPayload = {marketData: market.markets[0]}
    }else{
      analysisPayload = {marketData:market}
    }
